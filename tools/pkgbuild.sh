...
parse_pkgbuild() {
    ...
    validate_empty "$pkgbuild_source_name" "Package $pkgbuild_name pkgbuild file doesn't define pkgbuild_source_name"
    validate_empty "$pkgbuild_tarball_name" "Package $pkgbuild_name pkgbuild file doesn't define pkgbuild_tarball_name"
...
prepare_sources() {
    if [ ! -d "$pkgbuild_pkgdir/.src" ] || ! (cd "$pkgbuild_pkgdir" && sha512sum -c $pkgbuild_pkgdir/sources.sha512sum >/dev/null 2>&1); then
        ...
        info "Patching $pkgbuild_name sources..."
        if command -v pkgbuild_prepatch >/dev/null 2>&1; then
            (cd $pkgbuild_pkgdir && pkgbuild_prepatch)
        fi

        if [ -d "$pkgbuild_pkgdir/patches" ]; then
            for srcpatch in $(ls "$pkgbuild_pkgdir/patches"); do
                (cd "$pkgbuild_srcdir/$pkgbuild_source_name" && patch -p1 < "$pkgbuild_pkgdir/patches/$srcpatch")
            done
        fi
    fi
}
